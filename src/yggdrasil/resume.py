...
from .training import CAUSAL_LATENT_DROPOUT_FRACTION, FORMATION_OCCUPANCY_CEILING, HOME_T16_PROBE_STEPS, LIFE_VIABILITY_FLOOR, TrainingConfig, TrainingSummary, _rng_neutral_homeostasis_probe, _rng_neutral_homeostasis_trajectory, apply_latent_probe_intervention, attractor_trajectory_loss, attractor_trajectory_loss_per_sample, causal_latent_probe_mask, causal_latent_probe_mask_domain_separated, causal_latent_prune_terms, formation_hard_active_counts, formation_occupancy_ceiling_loss, trace_occupancy_ceiling_loss, balanced_hard_support_allocation_terms, decoupled_life_occupancy_ceiling_loss, decoupled_mature_sample_mask, frontier_life_floor_loss, mature_visible_anchor_terms, training_morphology_loss
...
class ResumableTrainingSession:
    ...
    def _one_iteration(self) -> None:
        ...
        if self.config.loss_mode == 'global_plus_foreground_bg_alpha_homeostasis':
        ...
        elif self.config.loss_mode == 'global_plus_foreground_bg_alpha_attractor_t16_life4_band113_800_causal_latent_dropout25_prune_t16':
            mature_mask = decoupled_mature_sample_mask(result, self.target_batch, state_alive_channel=self.model.config.alive_channel, target_alpha_channel=3, foreground_threshold=0.1, alive_threshold=self.model.config.alive_threshold)
            attractor_mature_samples = int(mature_mask.sum().item())
            ...
            if attractor_mature_samples > 0:
                mature_result = result[mature_mask]
                mature_target = self.target_batch[mature_mask]
                intact_trajectory = _rng_neutral_homeostasis_trajectory(model=self.model, result=mature_result, generator=self.device_rng)
                intact_future = intact_trajectory[1:]
                attractor_loss = attractor_trajectory_loss(intact_future, mature_target, visible_channels=self.config.visible_channels)
                probe_mask = causal_latent_probe_mask(mature_result.detach(), generator=self.device_rng, life_channel=self.model.config.alive_channel, alive_threshold=self.model.config.alive_threshold, fraction=CAUSAL_LATENT_DROPOUT_FRACTION)
                counterfactual_start = apply_latent_probe_intervention(mature_result.detach(), probe_mask, visible_channels=self.config.visible_channels)
                with torch.no_grad():
                    counterfactual_trajectory = _rng_neutral_homeostasis_trajectory(model=self.model, result=counterfactual_start, generator=self.device_rng)
                    intact_per_sample = attractor_trajectory_loss_per_sample(tuple(x.detach() for x in intact_future), mature_target.detach(), visible_channels=self.config.visible_channels)
                    counterfactual_per_sample = attractor_trajectory_loss_per_sample(counterfactual_trajectory[1:], mature_target.detach(), visible_channels=self.config.visible_channels)
                    causal_delta = (counterfactual_per_sample - intact_per_sample).detach()
                causal_prune_loss, _, classified_mask, nonbeneficial_mask, dropped_counts, actual_drop_fractions = causal_latent_prune_terms(mature_result, probe_mask, causal_delta, life_channel=self.model.config.alive_channel, alive_threshold=self.model.config.alive_threshold, maturity_floor=LIFE_VIABILITY_FLOOR)
            ...
            if attractor_mature_samples > 0:
                ...
                intact_future = intact_trajectory[1:]
                attractor_loss = attractor_trajectory_loss(intact_future, mature_target, visible_channels=self.config.visible_channels)
                probe_mask = causal_latent_probe_mask_domain_separated(mature_result.detach(), generator=self.device_rng, life_channel=self.model.config.alive_channel, alive_threshold=self.model.config.alive_threshold, fraction=CAUSAL_LATENT_DROPOUT_FRACTION)
                counterfactual_start = apply_latent_probe_intervention(mature_result.detach(), probe_mask, visible_channels=self.config.visible_channels)
                with torch.no_grad():
                    counterfactual_trajectory = _rng_neutral_homeostasis_trajectory(model=self.model, result=counterfactual_start, generator=self.device_rng)
                    intact_per_sample = attractor_trajectory_loss_per_sample(tuple(x.detach() for x in intact_future), mature_target.detach(), visible_channels=self.config.visible_channels)
        ...
        if iteration == 0 or iteration == self.config.iterations - 1 or (iteration + 1) % self.config.record_every == 0:
            ...
            if self.config.loss_mode == 'global_plus_foreground_bg_alpha_attractor_t16_life4_band113_800_visanchor_r1':
                ...
                item['life_channel'] = self.model.config.alive_channel
                item['visible_alpha_channel'] = 3
            if self.config.loss_mode == 'global_plus_foreground_bg_alpha_attractor_t16_life4_band113_800_causal_latent_dropout25_prune_t16':
                item['foreground_morphology_mse'] = float(foreground_morphology_mse(result, self.target_batch, visible_channels=self.config.visible_channels, alpha_channel=3, foreground_threshold=0.1).detach().item())
                item['background_alpha_mse'] = float(background_alpha_mse(result, self.target_batch, alpha_channel=3, foreground_threshold=0.1).detach().item())
                item['attractor_trajectory_loss'] = float(attractor_loss.detach().item()) if attractor_loss is not None else 0.0
                item['attractor_mature_samples'] = attractor_mature_samples
