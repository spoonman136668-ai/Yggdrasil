...
PREREG_LU2M="0b0a05f13bf053e45dfe616d7585102203e422b3"
...
def strong_lesion(seed):
    inherited=set(p.lesion_set(seed))
    strong=inherited|{(i+4)%p.N for i in inherited}
    if len(inherited)!=8 or len(strong)!=16 or not inherited < strong:
        raise AssertionError(("lesion geometry",len(inherited),len(strong)))
    return sorted(strong)

def derive_programs(lu2mf1,replicate):
    if not isinstance(lu2mf1,str) or len(lu2mf1)!=40:
    ...
    for stream in ("C","S"):
        ...
        for stage in ("A","B"):
            counter=0
            while True:
                material=f"LU2M-TASK2-L1-DOUBLE-LESION|{lu2mf1}|{replicate}|{stream}|{stage}|{counter}"
...
def _manifest(seed,programs,kind,replicate,lu2mf1,namespace):
    ...
    m={
        ...
        "replicate":replicate,
        "lu2mf1":lu2mf1,
        "lu2m_prereg_commit":PREREG_LU2M,
        "lu2m_seed_namespace":namespace,
...
def mechanical_manifest():
    seed="LU2M-MECHANICAL-DOUBLE-LESION-R1"
    return _manifest(seed,g.mechanical_programs(),"LU2M_MECHANICAL",0,None,seed)

def primary_manifest(lu2mf1,replicate):
    global _PRIMARY_MANIFEST_CALLS
    _PRIMARY_MANIFEST_CALLS+=1
    ns=f"LU2M-TASK2-L1-DOUBLE-LESION|{lu2mf1}|{replicate}"
    seed=hashlib.sha256(ns.encode()).hexdigest()[:32]
    return _manifest(seed,derive_programs(lu2mf1,replicate),"LU2M_PRIMARY",replicate,lu2mf1,ns)

def primary_manifests(lu2mf1):
    return [primary_manifest(lu2mf1,k) for k in range(1,REPLICATES+1)]

def validate_manifest(m):
    if m["manifest_sha256"]!=manifest_identity(m): raise AssertionError("manifest hash")
    if m["lu2m_prereg_commit"]!=PREREG_LU2M: raise AssertionError("prereg")
    if (m["parent_lu2l_closure"],m["parent_lu2lf1"],m["parent_lu2k_closure"],m["parent_v02"])!=(PARENT_LU2L_CLOSURE,PARENT_LU2LF1,PARENT_LU2K_CLOSURE,PARENT_V02):
    ...
    if m["lesion"]!=strong_lesion(m["seed"]): raise AssertionError("strong lesion")
    if len(m["inherited_lesion"])!=8 or len(m["lesion"])!=16 or not set(m["inherited_lesion"])<set(m["lesion"]): raise AssertionError("lesion size/subset")
    ...
    if m["kind"]=="LU2M_PRIMARY":
        rep=m["replicate"]; freeze=m["lu2mf1"]
        ns=f"LU2M-TASK2-L1-DOUBLE-LESION|{freeze}|{rep}"
        if m["lu2m_seed_namespace"]!=ns or m["seed"]!=hashlib.sha256(ns.encode()).hexdigest()[:32]: raise AssertionError("seed")
        if m["programs"]!=derive_programs(freeze,rep): raise AssertionError("primary programs")
    elif m["kind"]=="LU2M_MECHANICAL":
        if m["replicate"]!=0 or m["lu2mf1"] is not None: raise AssertionError("mechanical identity")
    else:
...
def qualification(sweep):
    ...
    for row in feasible:
        ...
        checks={
            ...
            "double_lesion_recovery":lesion_ok,
    ...
    return {
        "LU2M_DOUBLE_LESION_ROBUSTNESS":bool(positive),
...
def mechanical_gate():
    ...
    probes={
        ...
        "strong_lesion_16":len(strong)==16,
        "inherited_subset_strong":inherited < strong,
    ...
    return {"schema":1,"probes":probes,"all_pass":all(probes.values()),"weight_sha256":weight_sha,"inherited_lesion":sorted(inherited),"strong_lesion":sorted(strong),"pair_bytes":len(b1),"pair_sha256":hashlib.sha256(b1).hexdigest(),"mechanical_pair":pair1}
...
def open_duplicate(path1,path2):
    ...
    if b1!=b2: raise RuntimeError("LU2M duplicate mismatch")
