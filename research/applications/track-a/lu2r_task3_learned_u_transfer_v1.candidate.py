#!/usr/bin/env python3
import hashlib, inspect, json, math, sys
...
PREREG_LU2R="eff304b92d5d42349b79a94091132783596eb1e9"
PARENT_LU2Q_CLOSURE="b53159cbf2a87f1a8e7afaeb7232b2f63e41bc1d"
PARENT_LU2QF1="c2907a97aeff03f6fb7bebacf448fc9027859e19"
...
def derive_programs(lu2rf1,replicate):
    if not isinstance(lu2rf1,str) or len(lu2rf1)!=40: raise ValueError("freeze sha")
    ...
    for stream in ("C","S"):
        ...
        for stage in ("A","B","C"):
            counter=0
            while True:
                material=f"LU2R-TASK3-L1-A25|{lu2rf1}|{replicate}|{stream}|{stage}|{counter}"
                cand=int.from_bytes(hashlib.sha256(material.encode()).digest()[:2],"big")
                counter+=1
                if not q.valid_program(cand) or cand in used: continue
                used.add(cand); out[stream][stage]=cand; break
    return out

def _manifest(seed,programs,kind,replicate,lu2rf1,namespace):
    arrivals=q.make_arrivals(seed,programs)
    m={
        "kind":kind,"seed":seed,"replicate":replicate,"lu2rf1":lu2rf1,
        "lu2r_prereg_commit":PREREG_LU2R,"lu2r_seed_namespace":namespace,
        "parent_lu2q_closure":PARENT_LU2Q_CLOSURE,"parent_lu2qf1":PARENT_LU2QF1,
        "parent_lu2p_closure":PARENT_LU2P_CLOSURE,"parent_lu2l_closure":PARENT_LU2L_CLOSURE,
        "parent_v02":PARENT_V02,"canonical_weight_sha256":WEIGHT_SHA,"alpha":ALPHA,
...
def mechanical_manifest():
    seed="LU2R-MECHANICAL-TASK3-A25-R1"
    return _manifest(seed,q.mechanical_programs(),"LU2R_MECHANICAL",0,None,seed)

def primary_manifest(lu2rf1,replicate):
    global _PRIMARY_MANIFEST_CALLS
    _PRIMARY_MANIFEST_CALLS+=1
    ns=f"LU2R-TASK3-L1-A25|{lu2rf1}|{replicate}"
    seed=hashlib.sha256(ns.encode()).hexdigest()[:32]
    return _manifest(seed,derive_programs(lu2rf1,replicate),"LU2R_PRIMARY",replicate,lu2rf1,ns)

def primary_manifests(lu2rf1):
    return [primary_manifest(lu2rf1,k) for k in range(1,REPLICATES+1)]

def validate_manifest(m):
    if m["manifest_sha256"]!=manifest_identity(m): raise AssertionError("manifest hash")
    if m["lu2r_prereg_commit"]!=PREREG_LU2R: raise AssertionError("prereg")
    if (m["parent_lu2q_closure"],m["parent_lu2qf1"],m["parent_lu2p_closure"],m["parent_lu2l_closure"],m["parent_v02"])!=(PARENT_LU2Q_CLOSURE,PARENT_LU2QF1,PARENT_LU2P_CLOSURE,PARENT_LU2L_CLOSURE,PARENT_V02):
    ...
    if len(set(vals))!=6 or not all(q.valid_program(v) for v in vals): raise AssertionError("programs")
    if m["arrivals"]!=q.make_arrivals(m["seed"],m["programs"]): raise AssertionError("arrival binding")
    ...
    if m["kind"]=="LU2R_PRIMARY":
        rep=m["replicate"]; freeze=m["lu2rf1"]
        ns=f"LU2R-TASK3-L1-A25|{freeze}|{rep}"
        if m["lu2r_seed_namespace"]!=ns or m["seed"]!=hashlib.sha256(ns.encode()).hexdigest()[:32]: raise AssertionError("seed")
        if m["programs"]!=derive_programs(freeze,rep): raise AssertionError("primary programs")
    elif m["kind"]=="LU2R_MECHANICAL":
        if m["replicate"]!=0 or m["lu2rf1"] is not None: raise AssertionError("mechanical identity")
...
def _run_world(m,mode,instrument=True):
    ...
    def factory(rid,stream,created,bits,program_a,program_b,pos,scheduled_corrupt=False):
        r=q.Task3Request(rid,stream,created,bits,program_a,program_b,programs[stream]["C"],pos,scheduled_corrupt)
        registry[rid]=r
        return r

    def op(seed,t,cell,opname,r,trace):
        before=r.stage
        q.do_operation3(seed,t,cell,opname,r,trace)
    ...
    g.task2_op=q.task3_op
    g.do_operation=op
    g.expected_pair=q.expected_pair_compat
...
def maturity_summary(m,registry,detected):
    return q.maturity_summary(m,registry,detected)
...
def run_pair(m):
    ...
    return {
        ...
        "maturity":{"a0":maturity_summary(m,reg0,det0),"a25":maturity_summary(m,reg25,det25)},
        "causal":{
...
def qualification(sweep):
    ...
    return {
        "LU2R_TASK3_LEARNED_U_TRANSFER":bool(positive),
...
def mechanical_gate():
    ...
    stage=q.stage_repair_fixture()
    maturity=q.maturity_fixture()
    ...
    probes={
        "exact_task3_parent":q.PARENT_V02==PARENT_V02 and base.PARENT_V02==PARENT_V02,
        ...
        "paired_transition_variates_common":bool(common_rng),
...
def open_duplicate(path1,path2):
    ...
    if b1!=b2: raise RuntimeError("LU2R duplicate mismatch")
...
def main():
    ...
    raise SystemExit("usage: mechanical OUT | manifests LU2RF1 OUT | sweep MANIFESTS OUT | open SWEEP1 SWEEP2 OUT")
