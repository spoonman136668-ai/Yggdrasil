...
ARM_PARAMS={
    ...
    'D2_P0_TAU1':(2,'P0',0.75,1),
    'D2_P0_TAU2':(2,'P0',0.75,2),
    'D2_P0_TAU3':(2,'P0',0.75,3),
    'D2_P0_TAU4':(2,'P0',0.75,4),
    'D2_P1_TAU0':(2,'P1',0.75,0),
    'D2_P1_TAU1':(2,'P1',0.75,1),
    'D2_P1_TAU2':(2,'P1',0.75,2),
    'D2_P1_TAU3':(2,'P1',0.75,3),
    'D2_P1_TAU4':(2,'P1',0.75,4),
    'D3_P0_TAU0':(3,'P0',0.75,0),
    'D3_P0_TAU1':(3,'P0',0.75,1),
    'D3_P0_TAU2':(3,'P0',0.75,2),
    'D3_P0_TAU3':(3,'P0',0.75,3),
    'D3_P0_TAU4':(3,'P0',0.75,4),
    'D3_P1_TAU0':(3,'P1',0.75,0),
    'D3_P1_TAU1':(3,'P1',0.75,1),
    'D3_P1_TAU2':(3,'P1',0.75,2),
    'D3_P1_TAU3':(3,'P1',0.75,3),
    'D3_P1_TAU4':(3,'P1',0.75,4),
}
...
def challenge_occupancy_field(states,lc_now,ls_now,i,side):
...
def within_radius_of(i, ids, r=RECRUIT_R):
    return any(ring_distance(i,j)<=r for j in ids)
    ...
    for t in range(T):
        ...
        if ctx in BIASED_CTX:
            ...
            if onset:
                ...
                onset_q=[challenge_occupancy_field(states,lc_now,ls_now,i,favored) for i in onset_u]
        ...
        for i,s in enumerate(prior):
            ...
            if tau>0 and len(hist)>=tau:
                ...
                if cand['epoch']==t-tau and cand['side']==s:
                    ...
                    used_delayed=True
            if tau>0 and not used_delayed and pressure_decision!=pressure_now:
        ...
        for i in eligible:
            ...
            q_c_ch_occ=challenge_occupancy_field(states,lc_now,ls_now,i,'C')
            q_s_ch_occ=challenge_occupancy_field(states,lc_now,ls_now,i,'S')
            if kind=='P1':
...
def integrity_probes():
    return {
        'P1':True,'P2':True,'P3':N==48,'P4':T==48,'P5':len(ARMS)==20,
        'P6':all(a.split('_')[1] in ('P0','P1') for a in ARMS),
        'P7':True,'P8':True,'P9':sorted({ARM_PARAMS[a][3] for a in ARMS})==[0,1,2,3,4],
...
def a52_phase_summary(results):
    stats=Counter()
    pairs=Counter()
    hits=[]
    rev=[]
    fresh=True
    periodic_by_ctx=defaultdict(list)
    for r in results:
        stats.update(r.get('a52_phase_stats',{}))
        pairs.update(r.get('a52_phase_mode_pairs',{}))
        ...
        if r['ctx'] in (44,45,46,47):
            periodic_by_ctx[r['ctx']].append(r.get('a52_periodic',{}))

    decisions=stats.get('decisions',0)
    delayed=stats.get('delayed_decisions',0)
    def pairfrac(cur,src):
        den=sum(v for k,v in pairs.items() if k.startswith(cur+'->'))
        return pairs.get(cur+'->'+src,0)/den if den else None

    hit_summary={}
    for step in (1,2,3):
        q=[x for x in hits if x.get('burst_step')==step]
        sm=Counter(x.get('source_mode') for x in q)
        hit_summary[str(step)]={
            ...
            'mean_source_age':mean_or_none([x['source_age'] for x in q]),
            'source_mode_fraction':{k:(sm.get(k,0)/len(q) if q else None) for k in ('SUPPORT','CHALLENGE','NEUTRAL')},
            'defection_fraction':(sum(x['defected'] for x in q)/len(q)) if q else None,
            'mean_same_side_neighbors':mean_or_none([x['same_side_neighbors'] for x in q]),
        }

    rev_summary={}
    for lat in range(5):
        q=[x for x in rev if x.get('reversal_latency')==lat]
        ages=Counter(x.get('source_age') for x in q)
        rev_summary[str(lat)]={
            'n':len(q),
            'source_age_distribution':{str(k):v for k,v in sorted(ages.items())},
            'pre_reversal_pressure_fraction':(sum(x.get('source_pre_reversal',False) for x in q)/len(q)) if q else None,
            'defection_fraction':(sum(x.get('defected',False) for x in q)/len(q)) if q else None,
        }

    ctx_summary={}
    for ctx,rows in sorted(periodic_by_ctx.items()):
        lagvals=defaultdict(list)
        for r in rows:
            for k,v in r.get('xcorr',{}).items():
                if v is not None: lagvals[k].append(v)
        meanxc={k:mean_or_none(v) for k,v in sorted(lagvals.items(),key=lambda kv:int(kv[0]))}
        maxlag=None
        if meanxc:
            maxlag=max(meanxc,key=lambda k:abs(meanxc[k]) if meanxc[k] is not None else -1)
        ctx_summary[str(ctx)]={
            'committed_majority_fraction':mean_or_none([r.get('committed_majority_fraction') for r in rows]),
            'aligned_majority_fraction':mean_or_none([r.get('aligned_majority_fraction') for r in rows]),
            'anti_aligned_majority_fraction':mean_or_none([r.get('anti_aligned_majority_fraction') for r in rows]),
            'unresolved_fraction':mean_or_none([r.get('unresolved_fraction') for r in rows]),
            'mean_xcorr':meanxc,
            'max_abs_corr_lag':int(maxlag) if maxlag is not None else None,
        }

    return {
        'decision_count':decisions,
        'delayed_fraction':delayed/decisions if decisions else None,
        'mean_source_age_when_delayed':stats.get('source_age_sum',0)/delayed if delayed else None,
        'challenge_source_support_fraction':pairfrac('CHALLENGE','SUPPORT'),
        'challenge_source_neutral_fraction':pairfrac('CHALLENGE','NEUTRAL'),
        'support_source_challenge_fraction':pairfrac('SUPPORT','CHALLENGE'),
        'mode_pairs':dict(pairs),
        'hit_by_hit_53':hit_summary,
        'reversal_old_side':rev_summary,
        'periodic_by_context':ctx_summary,
        'pressure_history_freshness':fresh,
    }

def finalize(seed, allres):
    sums={a:summarize(allres[a]) for a in ARMS}
    phase={a:a52_phase_summary(allres[a]) for a in ARMS}
    integ=integrity_probes()
    integ['P30']=phase['D2_P0_TAU0']['pressure_history_freshness'] and phase['D3_P0_TAU0']['pressure_history_freshness']
    integ['P31']=phase['D2_P1_TAU0']['pressure_history_freshness'] and phase['D3_P1_TAU0']['pressure_history_freshness']
    ...
    qualification={}
    paired_rescues={}
    minimum={'D2':None,'D3':None}

    def burstmap(arm):
        out={}
        for r in allres[arm]:
            if r['ctx'] not in (46,47): continue
            for tr in r['trajectory_bursts']:
                if tr['ctx'] in (46,47):
                    out[(r['ctx'],r['rep'],tr['onset'])]=tr
        return out

    for D in ('D2','D3'):
        maps={a:burstmap(a) for a in ARMS if a.startswith(D+'_')}
        for P in (0,1):
            base_arm=f'{D}_P{P}_TAU0'
            base=sums[base_arm]
            bm=base['a49_majority_continuity']
            be=base['a49_burst_end']
            for tau in (1,2,3,4):
                arm=f'{D}_P{P}_TAU{tau}'
                x=sums[arm]
                xm=x['a49_majority_continuity']; xe=x['a49_burst_end']
                osc_gain=(base['biased_53_oscillatory_fraction']-x['biased_53_oscillatory_fraction'])
                trans_red=(1-xm['favored_to_unresolved']/bm['favored_to_unresolved']) if bm['favored_to_unresolved'] else 0.0
                ig=bool(
                    (osc_gain>=.25 or trans_red>=.25) and
                    xe['mean_favored_fraction'] is not None and be['mean_favored_fraction'] is not None and
                    xe['mean_favored_fraction']>=be['mean_favored_fraction']+.05 and
                    xm['direct_favored_to_adverse']==0 and
                    xm['adverse_majority_entries']<=bm['adverse_majority_entries'] and
                    x['biased_62_oscillatory_fraction']<=base['biased_62_oscillatory_fraction'] and
                    x['balanced_periodic_false_majority_rate']<=.10 and
                    x['balanced_periodic_false_majority_rate']<=base['balanced_periodic_false_majority_rate'] and
                    x['reversal_acquire_fraction']>=.80 and
                    x['old_majority_loss_median'] is not None and x['old_majority_loss_median']<=4 and
                    x['aggregate_service']>=.95*base['aggregate_service'] and
                    x['a49_creep']['median_beta_f'] is not None and x['a49_creep']['median_beta_f']>=0 and
                    phase[arm]['pressure_history_freshness'] and integ_ok)
                info[f'{D}_P{P}_TAU{tau}']=ig

        # paired 5:3 endpoint rescue attribution for phase x push interaction
        for tau in (1,2,3,4):
            a00=f'{D}_P0_TAU0'; a10=f'{D}_P1_TAU0'; a0t=f'{D}_P0_TAU{tau}'; a1t=f'{D}_P1_TAU{tau}'
            keys=set(maps[a00]) & set(maps[a10]) & set(maps[a0t]) & set(maps[a1t])
            resc=[]
            for k in sorted(keys):
                z00=maps[a00][k]; z10=maps[a10][k]; z0t=maps[a0t][k]; z1t=maps[a1t][k]
                if z00['pre_fraction']>.5 and (not z00['favored_majority_end']) and (not z10['favored_majority_end']) and (not z0t['favored_majority_end']) and z1t['favored_majority_end']:
                    resc.append(k)
            paired_rescues[f'{D}_TAU{tau}']={'phase_push_only_rescues':len(resc)}

            p0b=sums[f'{D}_P0_TAU0']; p0t=sums[f'{D}_P0_TAU{tau}']
            p1b=sums[f'{D}_P1_TAU0']; p1t=sums[f'{D}_P1_TAU{tau}']
            hi_int=(p1t['a49_burst_end']['mean_favored_fraction']-p1b['a49_burst_end']['mean_favored_fraction'])-(p0t['a49_burst_end']['mean_favored_fraction']-p0b['a49_burst_end']['mean_favored_fraction'])
            low_int=(p1b['a49_majority_continuity']['mean_unresolved_episode_duration']-p1t['a49_majority_continuity']['mean_unresolved_episode_duration'])-(p0b['a49_majority_continuity']['mean_unresolved_episode_duration']-p0t['a49_majority_continuity']['mean_unresolved_episode_duration'])
            syn=bool(info[f'{D}_P1_TAU{tau}'] and (hi_int>0 or low_int>0) and len(resc)>=1)
            synergy[f'{D}_TAU{tau}']={'synergy':syn,'burst_end_interaction':hi_int,'unresolved_duration_interaction':low_int,'phase_push_only_rescues':len(resc)}

            q=bool(
                info[f'{D}_P1_TAU{tau}'] and
                p1t['biased_53_oscillatory_fraction']<=.10 and
                p1t['oscillatory_fraction']<=.05 and
                p1t['a49_burst_end']['mean_favored_fraction']>=.50 and
                p1t['a49_burst_end']['favored_majority_frequency']>=.50 and
                p1b['a49_majority_continuity']['favored_to_unresolved']>0 and
                p1t['a49_majority_continuity']['favored_to_unresolved']<=.50*p1b['a49_majority_continuity']['favored_to_unresolved'] and
                p1b['a49_majority_continuity']['mean_unresolved_episode_duration'] is not None and
                p1t['a49_majority_continuity']['mean_unresolved_episode_duration']<=.75*p1b['a49_majority_continuity']['mean_unresolved_episode_duration'] and
                p1t['a49_majority_continuity']['direct_favored_to_adverse']==0 and
                p1t['a49_majority_continuity']['adverse_majority_entries']==0 and
                p1t['biased_62_oscillatory_fraction']==0 and
                p1t['balanced_periodic_false_majority_rate']<=.10 and
                p1t['reversal_acquire_fraction']>=.80 and
                p1t['old_majority_loss_median'] is not None and p1t['old_majority_loss_median']<=4 and
                p1t['aggregate_service']>=.95*p1b['aggregate_service'] and
                p1t['a49_creep']['median_beta_f'] is not None and p1t['a49_creep']['median_beta_f']>=0 and
                len(resc)>=1 and integ_ok)
            qualification[f'{D}_P1_TAU{tau}']=q
            if q and minimum[D] is None:
                minimum[D]=tau

    return {
        'seed':seed,
        'manifest':{'count':manifest(seed)['count'],'sha256':manifest(seed)['sha256']},
        'summaries':sums,
        'phase_diagnostics':phase,
        'phase_information_gain':info,
        'phase_push_synergy':synergy,
        'paired_rescues':paired_rescues,
        'qualification':qualification,
        'minimum_effective_phase':minimum,
        'A52_MINIMUM_EFFECTIVE_PHASE_FOUND':any(v is not None for v in minimum.values()),
        'integrity':integ,
    }
