...
PREREG="a1ddc3cfbfc1f8fc24d3ae25cfa79e7b6b4611db"
...
def test_lesion(seed):
    ...
    if TARGET_CELL in base:
        raise AssertionError("A27 ineligible seed")
    out=sorted(base+[TARGET_CELL])
    if len(out)!=12:
        raise AssertionError("A27 test cardinality")
    return out
...
def manifest_for(base,use_target):
    ...
    if a19.nonlesion_bytes(m)!=a19.nonlesion_bytes(base):
        raise AssertionError("A27 nonlesion mutation")
    return m
...
def run_mode(mode):
    ...
    return {
        ...
        "actual_replicate_ids_verified":eligible_reps==sorted(int(r["manifest"]["replicate"]) for r in anchor["rows"]),
...
def classify(rows):
    ...
    if not a0:
        return "NO_REPRODUCED_CELL2_EFFECT"
    if a0==[6]:
        return "REPLICATE6_CONTEXT_SPECIFIC"
    if len(a0)>=2:
        return "GLOBAL_CELL2_SENSITIVITY"
...
def main():
    ...
    out={
        ...
        "qualification":{
            "YGG_A27_CELL2_PORTABILITY_ATTRIBUTION":bool(all(validity.values()) and category in allowed),
