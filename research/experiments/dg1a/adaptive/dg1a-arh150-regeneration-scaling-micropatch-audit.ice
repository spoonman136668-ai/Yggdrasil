...
FINAL ACCEPTED PRIMARY CLOSURE — REGENERATION SCALING / MICRO-PATCH

DATE:
2026-09-19.

STATUS:
ACCEPTED PRIMARY COMPLETE / POSITIVE GEOMETRIC-RESOURCE RESULT.

FROZEN HARNESS SHA-256

51be6032bda5f3a42ca922bdef4cc62eb3d7f9c1c405380711fb0892b5fc646d.

ACCEPTED PRIMARY SEEDS

Axis A:
20260919600000..20260919612599.

Axis B:
20260919620000..20260919620999.

Axis C:
20260919630000..20260919637499.

Axis D:
20260919640000..20260919642399.
...
TOTAL:
23,500 accepted primary worlds.

Validation:

- every accepted axis contains the exact preregistered seed interval;
- accepted seeds are unique inside each axis;
- no T90 censoring occurred;
- diagnostic / later alternate-seed reruns are NON-EVIDENCE and are not used here.

ROBUSTNESS CONTROLS

Disjoint post-primary control family:
20260919695000...

3,600 control worlds.
...
No primary threshold was changed after:
the first accepted primary world.

AXIS A — BODY SIZE x PATCH FRACTION

Pooled median T90 by lesion fraction:
...
Within every tested body size,
pooled small lesions:

1%..5%

recover materially faster than:

20%..60%.

Small-versus-large pooled median T90:
...
LOCAL REGENERATION LATENCY
HAS A REAL
PATCH-SIZE ADVANTAGE.
...
Pooled median T90 by body radius:
...
A descriptive joint log-median fit:
...
These fitted exponents are:
descriptive scaling summaries,
not:
claimed universal laws.

No strongly superlinear body-size penalty
is observed.
...
Irregular wounds expose:
more frontier
and:
repair somewhat faster.

AXIS B — FIXED DAMAGE, MANY MICRO-PATCHES
...
Repair-opportunity cost changes by:
approximately 1%
rather than:
scaling upward with patch count.

Median macro downtime relative to:
one compact patch:
...
finer granularity reduces:
macro downtime

but:
requires more than:
20% additional repair opportunities

at:
g <=0.50.

Therefore:
it fails the frozen break-even rule.
...
with:
essentially equal repair work.

At:
beta=2.5,

finer granularity also begins to reduce:
T90 itself,

but:
H150 does NOT require
super-localization above 2.

INTERPRETATION

The result is not:

MORE CELLS ARE AUTOMATICALLY BETTER.
...
WHEN:
FUNCTIONAL DAMAGE LOCALIZATION
IMPROVES
APPROXIMATELY WITH
CELL AREA.
...
means:
the repair blast radius must shrink
roughly fast enough
to offset:
the extra cell count.

This is:
the central H150 architectural result.

AXIS D — PARALLEL REPAIR INTERFERENCE

R=30.

Each requested focus:
2% body-equivalent damage.

Median T90,
UNLIMITED versus BOUNDED 10% bandwidth:
...
bounded repair bandwidth
does NOT erase
the multi-patch advantage
at low / moderate concurrency.

The 10% bandwidth ceiling
begins to bind strongly
around:
16 simultaneous 2% wounds.
...
The requested 32 wounds
merge geometrically.

Median actual initial Moore components:
...
Therefore:
the 32-focus result
is a saturation regime,
not:
32 independent micro-patches.

This confirms:
there is a practical concurrency ceiling
set by:
both repair bandwidth
and:
spatial packing.

ROBUSTNESS — REPAIR PROBABILITY

Axis A retained:
the small-lesion advantage.
...
4+
spatially separated micro-patches
reduce:
median T90
relative to:
one compact patch.
...
6.
The small-cell / localization advantage
survives:
p=0.25
through:
p=1.0
...
downtime can improve,
but:
repair opportunity cost
remains too high.

3.
Repair bandwidth becomes:
a real concurrency bottleneck
at:
high simultaneous wound count.

4.
Spatial packing itself
limits:
how many independent patches
can exist
at:
high damaged fraction.
...
INCREASE CELL GRANULARITY

ONLY WHEN:
THE FAILURE / FUNCTIONAL BLAST RADIUS
SHRINKS
AT LEAST
APPROXIMATELY WITH
CELL AREA.
...
more,
smaller,
independently regenerable units

can:

- reduce macro functional downtime;
- preserve similar repair work;
- exploit parallel local repair;
- keep small failures local;
- avoid whole-region regeneration.

This directly supports:
testing
many small computational cells
as:
cognitive micro-patch units.

H150 DOES NOT ESTABLISH

- learned semantic regeneration;
- real cognitive specialization;
- endogenous injury localization;
- memory reconstruction;
- live capability replacement;
...
context dependence,
and:
whether local developmental neighbors
reconstruct:
lost specialization.
...
FINAL ACCEPTED PRIMARY CLOSURE — REGENERATION SCALING / MICRO-PATCH GRANULARITY

DATE:
2026-09-19.

STATUS:
ACCEPTED PRIMARY COMPLETE / POSITIVE GEOMETRIC-RESOURCE PLAUSIBILITY.

FROZEN HARNESS SHA-256

51be6032bda5f3a42ca922bdef4cc62eb3d7f9c1c405380711fb0892b5fc646d.

PRIMARY MATRIX INTEGRITY

Axis A:
12,600 / 12,600 unique held-out worlds.

Seed range:
20260919600000..20260919612599.

Axis B:
1,000 / 1,000 unique held-out worlds.

Seed range:
20260919620000..20260919620999.

Axis C:
7,500 / 7,500 unique held-out worlds.

Seed range:
20260919630000..20260919637499.

Axis D:
2,400 / 2,400 unique held-out worlds.

Seed range:
20260919640000..20260919642399.

Across all four primary axes:

- no duplicate accepted seed;
- no accepted-seed gap;
- no T90 censoring;
- no unrepaired final lesion.

PRIMARY AXIS A — BODY SIZE x PATCH FRACTION

Pooled median T90 by patch fraction:
...
Representative body-size effect
at:
5% damage:
...
median T90
across body size
within fixed patch / geometry cells

gave:
median body-size exponent
approximately 0.19,

with:
no strongly superlinear scaling.

Larger bodies therefore:
slow local repair,

but:
the measured latency growth
is sublinear
over the tested range.

Patch-size scaling:

median log-log T90 exponent
approximately:
0.22

across:
body / lesion-family cells.
...
the parallel-frontier latency gain
does NOT come from:
substantially increased total repair opportunity count.
...
PRIMARY AXIS C — GRANULARITY / LOCALIZATION BREAK-EVEN
...
with:
approximately 4x repair opportunities.
...
macro downtime improves,

but:
g<=0.50
still incurs:
>20% repair-opportunity penalty.

Therefore:
beta=1.5
does NOT satisfy:
the frozen break-even rule.
...
finer granularity
provides:
large functional-downtime reduction
...
micro-patch granularity becomes advantageous
when:
localizable damaged macro area
improves approximately:
in proportion to cell area.
...
to:
approximately 4 steps.

PRIMARY AXIS D — PARALLEL REPAIR INTERFERENCE

R=30.

2% body-equivalent lesion per requested focus.
...
BOUNDED 10%-OF-BODY OPPORTUNITY BANDWIDTH
median T90:
...
bounded repair bandwidth
does NOT erase:
the micro-patch advantage
at:
moderate concurrency.

The bandwidth knee appears:
between:
8 and 16
simultaneous 2%-body wounds
in this realization.
...
damage occupies:
approximately 64.7% of body

and:
geometric merging reduces:
median initial disconnected components
to:
4.

This is:
the preregistered saturation regime,

not:
32 independent wounds.
...
is below:
the R=30
single compact 60% lesion
median T90:
16.

ROBUSTNESS CONTROLS

Disjoint control seeds:
...
3.
Parallel micro-patch advantage
does not disappear completely
under:
bounded repair bandwidth.
...
in:
the frozen stratified controls.
...
ALSO SHRINKS
THE FUNCTIONAL / REPAIR BLAST RADIUS
...
more,
smaller,
independently repairable cells

can:
reduce functional downtime

without:
paying a larger repair-opportunity bill
for the same macro defect.

The second strong result is:

MULTIPLE SMALL WOUNDS
CAN REPAIR IN PARALLEL

through:
larger aggregate frontier area
...
whether:
a local computational specialization
can actually be:

- damaged;
- quarantined;
- regenerated from neighboring developmental context;
- restored functionally;
- and reintegrated

without:
restoring saved mutable state.
...
- learned cognitive specialization;
- injury localization;
- semantic capability restoration;
- memory inheritance;
- safe live cognitive replacement;
- production cell granularity.

It establishes:
the geometric / resource regime
in which:
