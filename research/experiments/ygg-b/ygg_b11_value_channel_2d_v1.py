...
import ygg_b9_role_subspace_v1 as b9

PREREG="877d650658f88072eb18992c9f4d1ad9cdef7205"
...
def mechanical():
    ...
    checks={
        ...
        "role_masks_exact_b9":torch.equal(MASK,b9.MASK),
        "cross_role_cosine_le_1e_7":cross<=1e-7,
    }
    return {
        "all_pass":all(checks.values()),
        "checks":checks,
        "max_cross_role_abs_address_cosine":cross,
...
def one_pass():
    ...
    q["YGG_B11_VALUE_CHANNEL_2D"]=all(q.values())
    for row in out["rows"]:
...
def main():
    ...
    if not mech["all_pass"]:
        raise SystemExit("YGG_B11_MECHANICAL_GATE_FAILED")
    p1=one_pass()
    p2=one_pass()
    b1=b6.canonical(p1)
    b2=b6.canonical(p2)
