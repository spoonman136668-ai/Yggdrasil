...
PREREG="073f03e27e3ef7fc3f4ab218efc4e25236efa875"
...
def structural_events(g,n):
    ...
    sel=torch.argsort(addr_scores,dim=2)[:,:,:2]
    ...
    for ent in range(3):
        base=2*ent
        keys[:,base:base+2]=entity_keys[:,ent].unsqueeze(1).expand(-1,2)
        roles[:,base:base+2]=COMBO_ROLES[sel[:,ent]]
        slots[:,base:base+2]=COMBO_SLOTS[sel[:,ent]]

    update_choice=torch.randint(0,2,(n,3),generator=g,dtype=torch.long)
    for ent in range(3):
        rows=torch.arange(n,dtype=torch.long)
        src=2*ent+update_choice[:,ent]
        dst=6+ent
        keys[:,dst]=keys[rows,src]
        roles[:,dst]=roles[rows,src]
        slots[:,dst]=slots[rows,src]

    query_entity=torch.randint(0,3,(n,),generator=g,dtype=torch.long)
    return keys,roles,slots,entity_keys,update_choice,query_entity

def pack(keys,roles,slots,vals,query_entity):
    n=keys.shape[0]
    rows=torch.arange(n,dtype=torch.long)
    qevent=6+query_entity
    qk=keys[rows,qevent]
    ...
    x[:,36]=QUERY_KEY_BASE+qk
    x[:,37]=QUERY_ROLE_BASE+qr
    x[:,38]=QUERY_SLOT_BASE+qs
...
def make_data(seed,n,evaluation):
    ...
    g.manual_seed(seed*100000+(72 if evaluation else 71))
    keys,roles,slots,entity_keys,update_choice,query_entity=structural_events(g,n)
    ...
    for row in range(n):
        for ent in range(3):
            old_event=2*ent+int(update_choice[row,ent].item())
            new_event=6+ent
    ...
    return x,target,keys,roles,slots,vals,entity_keys,update_choice,query_entity,qk,qr,qs
...
VALUE_COLS=torch.tensor([3,7,11,15,19,23,27,31,35],dtype=torch.long)
ROLE_COLS=torch.tensor([1,5,9,13,17,21,25,29,33],dtype=torch.long)
SLOT_COLS=torch.tensor([2,6,10,14,18,22,26,30,34],dtype=torch.long)

def query_order_swap(x):
    out=x.clone()
    for i in range(x.shape[0]):
        qk=int(x[i,36]-QUERY_KEY_BASE)
        qr=int(x[i,37]-QUERY_ROLE_BASE)
        qs=int(x[i,38]-QUERY_SLOT_BASE)
        matches=[]
...
class ThreeEntityTemporalBinding(nn.Module):
    def __init__(self):
        super().__init__()
...
def train_seed(seed):
    ...
    model=ThreeEntityTemporalBinding()
    opt=torch.optim.Adam(model.parameters(),lr=LR)
    loss_fn=nn.CrossEntropyLoss()

    xtr,ytr,ktr,rtr,str_,vtr,ektr,uctr,qetr,qktr,qrtr,qstr=training_data(seed,TRAIN_N)
    xev,yev,kev,rev,sev,vev,ekev,ucev,qeev,qkev,qrev,qsev=evaluation_data(seed,EVAL_N)
    ...
    for ent in range(3):
        old_event=2*ent+ucev[:,ent]
        old_vals=vev[rows,old_event]
        if torch.any(old_vals==vev[:,6+ent]):
            raise AssertionError("overwrite values not distinct")

    gen=torch.Generator(device="cpu")
    gen.manual_seed(seed*100000+73)
...
def mechanical():
    ...
    model=ThreeEntityTemporalBinding()
    g=torch.Generator(device="cpu")
    g.manual_seed(991301)
    keys,roles,slots,entity_keys,update_choice,qe=structural_events(g,1024)
    ...
    for i in range(keys.shape[0]):
        ...
        for ent in range(3):
            ...
            if int(keys[i,a])!=key or int(keys[i,b])!=key:
                exact=False; break
            if len({
                (int(roles[i,a]),int(slots[i,a])),
                (int(roles[i,b]),int(slots[i,b]))
            })!=2:
                exact=False; break
            src=2*ent+int(update_choice[i,ent].item())
            dst=6+ent
            if int(keys[i,dst])!=key:
                exact=False; break
    ...
    checks={
        ...
        "six_active_bindings_exact":ACTIVE_BINDINGS==6,
        "nine_writes_exact":WRITES==9,
        "projection_overwrite_formula_exact":bool(torch.allclose(isolated,v2,atol=1e-6,rtol=0.0)),
        "sequence_length_exactly_39":SEQ==39,
...
def one_pass():
    ...
    q["YGG_B25_THREE_ENTITY_TEMPORAL_OVERWRITE"]=all(q.values())
...
def main():
    ...
    if not mech["all_pass"]:
        raise SystemExit("YGG_B25_MECHANICAL_GATE_FAILED")
    p1=one_pass()
    p2=one_pass()
