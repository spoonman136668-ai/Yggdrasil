...
PREREG="772e247101e692bfcb6832d5ab6b0583dc6cf94f"
...
def structural_addresses(g,n):
    ...
    keys=key.unsqueeze(1).expand(-1,BINDINGS).clone()
    roles=torch.tensor((0,0,1,1),dtype=torch.long).unsqueeze(0).expand(n,-1).clone()
    slots=torch.tensor((0,1,0,1),dtype=torch.long).unsqueeze(0).expand(n,-1).clone()
...
def train_seed(seed):
    ...
    gen.manual_seed(seed*100000+23)
...
def mechanical():
    ...
    for r in range(ROLES):
        for s in range(SLOTS):
            masks.append(tuple((ROLE_MASK[r]*SLOT_MASK[s]).tolist()))
    x,_,keys,roles,slots,_,_,_,_,_=training_data(991,32)
    checks={
        ...
        "four_structural_mask_products_distinct":len(set(masks))==4,
        "one_key_per_sequence":all(len(set(row.tolist()))==1 for row in keys),
        "one_sequence_has_all_four_addresses":all(
            set(zip(roles[i].tolist(),slots[i].tolist()))=={(0,0),(0,1),(1,0),(1,1)}
...
def one_pass():
    ...
    q["YGG_B14_REDUCED_LOAD_TWO_FACTOR_BINDING"]=all(q.values())
...
def main():
    ...
    if not mech["all_pass"]:
        raise SystemExit("YGG_B14_MECHANICAL_GATE_FAILED")
    p1=one_pass()
    p2=one_pass()
    b1=canonical(p1)
    b2=canonical(p2)
