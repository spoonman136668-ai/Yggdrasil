...
def step(state,bit,mapped,rules,method,ref_logical=None,tel=None):
    ...
    repair(state,method,ref_logical,tel); logical=decode(state); logical[mapped]=np.uint8(bit); logical=eca_step_mixed(logical,rules); state[:]=encode(logical)
    return tel
def prefix(u,mapped,rules,method):
    s=encode(np.zeros(LOGICAL_BITS,dtype=np.uint8))
