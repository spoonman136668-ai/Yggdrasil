...
def all_metrics(edges, include_geometry=False):
    ...
    for mask in range(1,1<<BLOCKS):
        ...
        sw=s.bit_count(); lw=mask.bit_count()
        syn.append((mask,lw,s,sw)); by_w[lw].append(sw)
        if s==0: kernel.append(mask)
    ...
    if include_geometry:
        ...
        mult[0] += 1  # include zero logical mask for full 4096 mapping
        ...
        o['minimum_positive_by_logical_weight']={
            str(w): ({'minimum':min(x for x in by_w[w] if x>0) if any(x>0 for x in by_w[w]) else None,
...
def transfer_candidate(block, src, dst):
    ...
    edges[src].remove(block); edges[dst].append(block)
    ...
    for b in range(BLOCKS):
        for i,e in enumerate(BASE_EDGES):
            ...
            for j,t in enumerate(BASE_EDGES):
                ...
                feasible=(m['rank']==12 and m['kernel_count']==0 and m['max_pair_cooccurrence']<=2 and m['single_floor']>=6 and m['two_floor']>=8)
                rows.append({'block':b,'source_check':i,'target_check':j,'metrics':m,'feasible':bool(feasible)})
    return rows
...
def select_candidate(rows):
    ...
    return sorted(fs,key=lambda r:(-r['metrics']['global_positive_floor'],-r['metrics']['two_floor'],-r['metrics']['single_floor'],r['metrics']['max_pair_cooccurrence'],r['block'],r['source_check'],r['target_check']))[0]
...
def check_only_boundary(edges):
    ...
    for w in range(1,5):
        ...
        out[str(w)]={'patterns':n,'zero_unique':c['zero_unique'],'zero_tied':c['zero_tied'],'all_minima_nonzero':c['all_minima_nonzero']}
    return out
...
def check4_tests(edges):
    ...
    for name,b in [('CHECK4_INCIDENT_D6',5),('CHECK4_INCIDENT_D7',2)]:
        ...
        out[name]={'block':b,'checks':xs,'classification':classify(s,0,syndromes,lw,pop)}
    return out
...
def crossplane(edges):
    ...
    for b in range(BLOCKS):
        inc=incident(edges,b); check_mask=sum(1<<i for i in inc)
        for p in range(1,PLANES+1):
            # same per-plane syndrome replicated across p planes
...
def baseline_exact():
    ...
    assert m['rank']==11 and m['kernel_count']==1 and m['kernel_masks']==[4095]
    assert m['single_floor']==6 and m['two_floor']==8 and m['global_positive_floor']==3
...
def run():
    ...
    out={'schema':'yggdrasil.dg1r-05k-parity-broken-topology.v1','mechanical_valid':True,'candidate_count':len(rows),'feasible_count':sum(r['feasible'] for r in rows),'baseline':base,'selected':None,'signals':{},'canonical_scientific_execution':False,'stab18_r1_touched':False}
    if selected is None:
        out['signals']={'parity_broken_feasible':False,'selected_full_rank':False,'selected_kernel_free':False,'local_floors_preserved':False,'pair_cooccurrence_preserved':False,'global_distance_improved':False,'check4_equal_cost_not_worse':False,'crossplane_arbitrary_correlation_solved':False}
        return out
    ed=transfer_candidate(selected['block'],selected['source_check'],selected['target_check'])
    sm=all_metrics(ed,True)
    hold=check_only_boundary(ed); base_hold=check_only_boundary(BASE_EDGES)
    c4=check4_tests(ed); base_c4=check4_tests(BASE_EDGES)
    cross=crossplane(ed)
    weight4=hold['4']['all_minima_nonzero']; base_weight4=base_hold['4']['all_minima_nonzero']
    out['selected']={
        'move':{'block':selected['block'],'source_check':selected['source_check'],'target_check':selected['target_check']},
...
def main():
    ...
    print(enc({'output':a.out,'sha256':hashlib.sha256(raw).hexdigest(),'candidate_count':o['candidate_count'],'feasible_count':o['feasible_count'],'signals':o['signals'],'selected_move':None if o['selected'] is None else o['selected']['move'],'selected_metrics':None if o['selected'] is None else o['selected']['metrics']}))
