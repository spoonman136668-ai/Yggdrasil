...
def record_correction(state,ref_pre,idx,tel,local_pos):
    tel["corrections"]+=1; tel["targets"][str(local_pos)]+=1
    ...
    for b in range(BLOCKS):
        ...
        if method in ("HAM74_ALWAYS","HAM74_H6_GATE"):
            ...
            if syn:
                ...
                if do: record_correction(state,ref_pre,s+syn-1,tel,syn)
        ...
        if syn!=0 and parity==1:
            record_correction(state,ref_pre,s+syn-1,tel,syn)
        elif syn==0 and parity==1:
            record_correction(state,ref_pre,s+7,tel,8)
        else: # syn != 0 and parity == 0 -> detected double
            tel["detected_double"]+=1
            if method=="SECDED84_FORCE": record_correction(state,ref_pre,s+syn-1,tel,syn)
            else: tel["abstentions"]+=1
