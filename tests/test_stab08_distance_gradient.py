...
from yggdrasil.metrics import (
    ...
    graded_background_alpha_mse,
    morphology_mse,
    target_chebyshev_distance_weights,
)
...
MODE = "global_plus_foreground_graded_bg_alpha"


def test_distance_weights_are_exact_chebyshev_rings() -> None:
    target = torch.zeros((1, 4, 7, 7), dtype=torch.float32)
    target[:, 3, 3, 3] = 1.0
    weights = target_chebyshev_distance_weights(target)
    assert weights[0, 0, 3, 3].item() == pytest.approx(0.0)
    assert weights[0, 0, 3, 4].item() == pytest.approx(0.5)
    assert weights[0, 0, 2, 2].item() == pytest.approx(0.5)
    assert weights[0, 0, 3, 5].item() == pytest.approx(2.0 / 3.0)
    assert weights[0, 0, 0, 0].item() == pytest.approx(3.0 / 4.0)


def test_distance_weights_fail_closed_without_foreground() -> None:
    target = torch.zeros((1, 4, 5, 5), dtype=torch.float32)
    with pytest.raises(ValueError, match="no foreground"):
        target_chebyshev_distance_weights(target)


def test_distance_weights_fail_closed_without_background() -> None:
    target = torch.zeros((1, 4, 5, 5), dtype=torch.float32)
    target[:, 3] = 1.0
...
def test_radius6_target_distance_cardinality_and_mean_weight_are_frozen() -> None:
    ...
    assert int(background.sum().item()) == 1487
    expected_counts = {1:56,2:64,3:72,4:80,5:88,6:96,7:104,8:112,9:120,10:128,11:136,12:144,13:152,14:102,15:28,16:5}
    for distance, count in expected_counts.items():
        value = distance / (distance + 1.0)
        observed = torch.isclose(weights, torch.tensor(value, dtype=weights.dtype), atol=1e-7, rtol=1e-6) & background
        assert int(observed.sum().item()) == count
    assert weights.masked_select(background).mean().item() == pytest.approx(0.8635463277, rel=1e-6)


def test_graded_background_alpha_formula() -> None:
    state = torch.zeros((1, 4, 5, 5), dtype=torch.float32)
    target = torch.zeros_like(state)
    target[:, 3, 2, 2] = 1.0
    state[:, 3, 2, 3] = 2.0
    state[:, 3, 0, 0] = 3.0
    value = graded_background_alpha_mse(state, target)
    expected = ((0.5 * 4.0) + ((2.0 / 3.0) * 9.0)) / 24.0
    assert value.item() == pytest.approx(expected)


def test_dist1_exact_training_formula() -> None:
    state = torch.zeros((1, 4, 5, 5), dtype=torch.float32)
    target = torch.zeros_like(state)
    target[:, :, 2, 2] = 1.0
    state[:, 3, 0, 0] = 0.5
    ...
    expected = morphology_mse(state, target, visible_channels=4) + foreground_morphology_mse(state, target, visible_channels=4) + graded_background_alpha_mse(state, target)
    assert actual.item() == pytest.approx(expected.item())
    assert torch.isfinite(actual)


def test_dist1_dead_state_sanity_matches_preregistration() -> None:
    target = make_target(batch_size=1, channels=16, height=40, width=40, spec=TargetSpec(radius=6))
    state = torch.zeros_like(target)
    value = training_morphology_loss(result=state, target=target, config=TrainingConfig(loss_mode=MODE, visible_channels=4))
    assert value.item() == pytest.approx(0.44832421875, rel=1e-6)


def test_stab08_config_validates_and_freezes_dist1_envelope() -> None:
    config = load_config(ROOT / "experiments/dg1a/configs/p0_stab08_dist1.yaml")
...
def _tiny_fixture():
    ...
    config = TrainingConfig(variant="regeneration", iterations=2, learning_rate=1e-3, steps_min=2, steps_max=2, batch_size=2, pool_size=4, damage_probability=0.5, damage_min_active_cells=4, loss_mode=MODE, record_every=1, seed=7)
    return model, seed, target, config


def test_train_records_dist1_component_telemetry() -> None:
    ...
    assert "graded_background_alpha_mse" in summary.history[-1]
    assert summary.history[-1]["graded_background_alpha_mse"] >= 0.0


def test_resumable_training_records_dist1_component_telemetry() -> None:
    model, seed, target, config = _tiny_fixture()
...
def test_runner_evidence_records_dist1_mode_and_telemetry(tmp_path: Path) -> None:
    config = {
    ...
    config_path = tmp_path / "tiny_stab08.yaml"
    evidence_path = tmp_path / "tiny_stab08.json"
    ...
    assert "graded_background_alpha_mse" in evidence["training"]["history"][-1]
