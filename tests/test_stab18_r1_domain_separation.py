...
MODE = "global_plus_foreground_bg_alpha_attractor_t16_life4_band113_800_causal_latent_dropout25_prune_t16_domainsep"
...
def test_domain_probe_seed_contract_and_main_rng_is_unchanged() -> None:
    state = _state(size=9)
    state[:, 4] = 0.2
    ...
    assert CAUSAL_PROBE_RNG_DOMAIN == b"DG1A_P0_STAB18_R1_CAUSAL_PROBE_V1"
    assert causal_probe_domain_seed(generator) == 3494006018084941813

    mask = causal_latent_probe_mask_domain_separated(state, generator=generator)

    assert bool(mask.any())
    assert torch.equal(generator.get_state(), before)


def test_domain_probe_is_not_aliased_to_first_future_fire_field() -> None:
    state = _state(size=9)
    state[:, 4] = 0.2
    generator = torch.Generator(device="cpu").manual_seed(1)
    before = generator.get_state().clone()
    ...
    first_future_random = torch.rand((1, 1, 9, 9), generator=shadow)
    probe = causal_latent_probe_mask_domain_separated(state, generator=generator)

    assert bool((probe & (first_future_random > 0.5)).any())
    assert not torch.equal(probe, first_future_random < 0.25)
    assert torch.equal(generator.get_state(), before)


def test_domain_probe_is_reproducible_and_changes_with_main_rng_state() -> None:
    state = _state(size=9)
    state[:, 4] = 0.2

    first_a = torch.Generator(device="cpu").manual_seed(1)
    first_b = torch.Generator(device="cpu").manual_seed(1)
    second = torch.Generator(device="cpu").manual_seed(2)

    mask_a = causal_latent_probe_mask_domain_separated(state, generator=first_a)
    mask_b = causal_latent_probe_mask_domain_separated(state, generator=first_b)
    mask_c = causal_latent_probe_mask_domain_separated(state, generator=second)

    assert torch.equal(mask_a, mask_b)
    assert not torch.equal(mask_a, mask_c)
...
def test_stab18_r1_full_objective_formula() -> None:
...
def test_stab18_r1_requires_causal_prune_auxiliary() -> None:
    result = torch.zeros((1, 8, 9, 9), dtype=torch.float32)
    target = make_target(batch_size=1, channels=8, height=9, width=9, spec=TargetSpec(radius=2))
    cfg = TrainingConfig(loss_mode=MODE, visible_channels=4)
...
def test_train_records_stab18_r1_telemetry() -> None:
...
def test_resumable_records_stab18_r1_telemetry() -> None:
    model, seed, target, config = _tiny_fixture()
...
def test_stab18_r1_frozen_config() -> None:
    config = load_config(ROOT / "experiments/dg1a/configs/p0_stab18_r1_life4_band113_800_causal_latent_dropout25_prune_t16_domainsep.yaml")
...
def test_resumable_pool_never_stores_stab18_r1_virtual_trajectory(monkeypatch) -> None:
...
def test_runner_evidence_records_stab18_r1_mode_and_telemetry(tmp_path: Path) -> None:
    config = {
