...
MODE = "global_plus_foreground_bg_alpha"


def test_background_alpha_mse_uses_target_background_and_alpha_only() -> None:
    ...
    value = background_alpha_mse(state, target, alpha_channel=3)

    assert value.item() == pytest.approx(4.0 / 3.0)


def test_background_alpha_mse_rejects_missing_background() -> None:
    state = torch.zeros((1, 4, 2, 2), dtype=torch.float32)
    target = torch.ones_like(state)

    with pytest.raises(ValueError, match="no background"):
        background_alpha_mse(state, target, alpha_channel=3)


def test_occ_a1_exact_formula_is_finite() -> None:
    state = torch.zeros((1, 4, 2, 2), dtype=torch.float32)
    target = torch.zeros_like(state)
    target[:, :, 0, 0] = 1.0
    state[:, 3, 0, 1] = 0.5
...
def test_occ_a1_dead_state_sanity_matches_preregistration() -> None:
    target = make_target(
        batch_size=1,
        channels=16,
        height=40,
...
def test_stab05_config_validates_and_freezes_occ_a1_envelope() -> None:
    config = load_config(ROOT / "experiments/dg1a/configs/p0_stab05_occ_a1.yaml")
...
def test_train_records_occ_a1_component_telemetry() -> None:
    ...
    assert "background_alpha_mse" in summary.history[-1]
    assert summary.history[-1]["background_alpha_mse"] >= 0.0


def test_resumable_training_records_occ_a1_component_telemetry() -> None:
    model, seed, target, config = _tiny_fixture()
...
def test_runner_evidence_records_occ_a1_mode_and_telemetry(tmp_path: Path) -> None:
    config = {
    ...
    config_path = tmp_path / "tiny_stab05.yaml"
    evidence_path = tmp_path / "tiny_stab05.json"
    ...
    assert "background_alpha_mse" in evidence["training"]["history"][-1]
